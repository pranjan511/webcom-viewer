 ...
 *
 * Dirty changes made for Webcom :
 * - Add a pseudo "transaction" (not yet tested on Webcom) method that's don't make a real transaction
 * - Remove calls to getPriority (not yet implemented on Webcom)
 * - Force aliases Webcom.* to Firebase.*
 *
 * AngularFire 1.0.X
...
(function() {
  Firebase=Webcom;
  Webcom.prototype.transaction=function(updateFunction, onComplete, applyLocally) {
    this.once(value, function(snapshot) {
      var res=updateFunction(snapshot);
        if (res!=null) 
          this.set(res, onComplete)
    });
      ...
      FirebaseArray.prototype = {
        ...
        $$added: function(snap/*, prevChild*/) {
          ...
          if( i === -1 ) {
            ...
            rec.$priority = null; //snap.getPriority();
